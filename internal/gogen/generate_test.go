...
func TestGenerateIgnoresUnrelatedUnsupportedPlatformDownload(t *testing.T) {
	target := manifest.RuntimeTarget()
	other := manifest.Target{GOOS: "linux", GOARCH: "amd64"}
	if target == other {
		other = manifest.Target{GOOS: "darwin", GOARCH: "arm64"}
	}
	digest := digestFor(t, "script")
	repo := testrepo.New(t, fmt.Sprintf(`schema: 1
resources:
  web:
    version: "1.0.0"
    downloads:
      script:
        url: https://example.com/${version}/script
        path: assets/vendor/script
        integrity: %s
  tool:
    version: "1.0.0"
    downloads:
      cli:
        ...
        platforms:
          %s:
            url: https://example.com/${version}/tool
            integrity: %s
`, digest, other.String(), digest))
	repo.Write(t, "assets/vendor/script", []byte("script"))
	repo.Write(t, "assets/doc.go", []byte("package assets\n"))
	doc, err := manifest.Load(repo.Manifest)
	if err != nil {
		t.Fatal(err)
	}
	if err := Generate(doc, Options{Dir: "assets", Output: "muamba_gen.go", Strict: true, Target: target}); err != nil {
		t.Fatalf("Generate failed for unrelated %s download: %v", other, err)
	}
}
